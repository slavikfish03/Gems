...
void Game::Run(Board& board) {
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            case sf::Event::MouseButtonPressed:
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    if (x >= 0 && x < board.GetGems()[0].size() && y >= 0 && y < board.GetGems().size()) {
                        Gem& current_gem = board.GetGemAt(x, y);  // выбираем текущий гем
                        if (selectable_gem != nullptr) {
                            // если предыдущий нажатый гем был этот же, снимаем выделение
                            if (selectable_gem == &current_gem) { 
                                selectable_gem->SetDeselected();
                                selectable_gem = nullptr;
                                break;
                            }
                            // если предыдущий нажатый гем сосед, меняем местами
                                ...
                                // если после обмена возникла комбинация "три в ряд", обновляем доску
                                if (board.IsFoundCombinatioins()) {
                                    board.Update(window);
                                }
                                // иначе возвращаем гемы на место
                                else {
                                    board.AnimationMove(selectable_gem->GetX(), selectable_gem->GetY(), current_gem.GetX(), current_gem.GetY(), 0.2f, window);
                                    board.SwapGems(selectable_gem->GetX(), selectable_gem->GetY(), current_gem.GetX(), current_gem.GetY());
                                }
            ...
            // создание вектора для бонусов-фломастеров
            ...
            // активация всех бонусов-фломастеров
            ...
            // создание вектора для бонусов-бомб
            ...
            // поиск комбинаций после активаций бонусов
