...
void Board::CreateGems() {
	// функция для создания гемов, создает доску без комбинаций
...
void Board::AnimationMove(int x1, int y1, int x2, int y2, float transitionTime, sf::RenderWindow& window) {
	// функция для анимации перемещения гема из точки (x1, y1) в точку (x2, y2)
...
bool Board::IsFoundCombinatioins() {
	// функция для поиска наличия комбинаций "три в ряд" на доске
...
std::set<std::tuple<int, int>> Board::SearchCombinations() {
	// функция для получения координат гемов, участвующих в комбинациях 
...
void Board::DestroyGems(std::set<std::tuple<int, int>>& coordinates_combinations, sf::RenderWindow& window) {
	// функция для уничтожения гемов после совершения комбинации
	// при уничтожении гема может выпасть бонус
...
void Board::AddGems(sf::RenderWindow& window) {
	// функция для добавления гемов на доску после уничтожения
	// гемы добавляются визуально сверху вниз
...
std::tuple<int, int> Board::SetBonusPosition(int x, int y) {
	// функция для получения координат для расположения
	// бонуса на расстоянии трех гемов от разрушенного
...
sf::VertexArray Board::CreateLines() {
	// функция для создания разлиновки доски
...
void Board::Update(sf::RenderWindow& window) {
	// функция для обновления доски: если есть комбинации, то
	// они уничтожаются, добавляются новые гемы, и так до тех
	// пор, пока комбинации не исчерпаются
